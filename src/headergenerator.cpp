...
// padding indicator in the actions vector
std::string g_padding = "_ahg_pad_";

bool HeaderGenerator::GenerateHeader(char *p_interleafName, std::vector<std::string> *p_actionVector, char *p_outputDir)
{
...
bool HeaderGenerator::WriteHeader(char *p_interleafName, std::vector<std::string> *p_actionVector)
{
    // create boilerplate
    ...
    for (int i = 0; i < vecSize; i++) {
        if (p_actionVector->at(i).compare(g_padding) == 0) {
            // this action is padding, so 
            // it's probaly not important
            // we'll just skip it

            // check if the previous action was also skipped so we
            // don't stack crazy amounts of whitespace into the enum
            if (i != 0) {
                if (p_actionVector->at(i - 1).compare(g_padding) != 0) {
                    m_fout << "\n";
                }
            }
